...
const LoginScreen = ({ onLogin, navigateTo }) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      try {
        ...
        if (role === 'Doctor') {
          ...
          if (status === 'Rejected') {
            alert('❌ Your account was rejected by the Admin. Please contact support.');
            setLoading(false);
            return;
          }
        }
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setTimeout(() => { setShowReset(false); setResetMessage(''); setResetEmail(''); }, 5000);
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        .login-wrapper { min-height: 100vh; display: flex; justify-content: center; align-items: center; background: url('https://images.unsplash.com/photo-1538108149393-fbbd81895907?auto=format&fit=crop&w=1000&q=80') no-repeat center center fixed; background-size: cover; padding: 20px; position: relative; }
        ...
        .login-card { background: #f9fafb; width: 100%; max-width: 350px; padding: 30px; border-radius: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.15); border: 1px solid #e2e8f0; }
        ...
        .modal-card { background: white; border-radius: 16px; padding: 30px; max-width: 400px; width: 100%; box-shadow: 0 20px 60px rgba(0,0,0,0.3); position: relative; }
        .modal-close { position: absolute; top: 15px; right: 20px; background: none; border: none; font-size: 24px; cursor: pointer; color: #888; }
        ...
        .modal-btn { width: 100%; padding: 12px; background: #667eea; color: white; border: none; border-radius: 8px; font-weight: 700; cursor: pointer; }
        ...
        .modal-cancel { display: inline-block; margin-top: 10px; color: #64748b; cursor: pointer; font-size: 13px; }
      `}</style>
