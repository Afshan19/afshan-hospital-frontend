  ...
  return (
    <>
      ...
      <div className="wrapper">
        ...
        <h1>Appointments</h1>
        <div className="table">
          {loading ? <div className="empty">Loading...</div> : data.length === 0 ? <div className="empty">No appointments scheduled.</div> : <table><thead><tr><th>Patient</th><th>Doctor</th><th>Date</th><th>Time</th></tr></thead><tbody>{data.map((d) => <tr key={d.id}><td><strong>{d.patient}</strong></td><td>{d.doctor}</td><td>{d.date}</td><td>{d.time}</td></tr>)}</tbody></table>}
        </div>
      </div>
    </>
  );
