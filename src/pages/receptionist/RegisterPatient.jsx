    ...
    <>
      <style>{`
        * { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input, textarea { width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 10px; font-size: 14px; transition: 0.2s; background: #f8fafc; font-family: inherit; }
        input:focus, textarea:focus { border-color: #3b82f6; outline: none; background: white; box-shadow: 0 0 0 3px rgba(59,130,246,0.15); }
      ...
      <div className="wrapper">
        ...
        <div className="card"><form onSubmit={handleSubmit}>
          ...
          <div className="form-group"><label>Address</label><textarea rows="2" value={form.address} onChange={(e) => setForm({...form, address: e.target.value})} placeholder="123 Main St, City" /></div>
          <button type="submit" className="btn" disabled={loading}>{loading ? 'Registering...' : 'Register Patient'}</button>
        </form></div>
        <div className="table">{patients.length === 0 ? <div className="empty">No patients registered yet.</div> : <table><thead><tr><th>Name</th><th>Age</th><th>Phone</th></tr></thead><tbody>{patients.map((p) => <tr key={p.id}><td><strong>{p.name}</strong></td><td>{p.age}</td><td>{p.phone}</td></tr>)}</tbody></table>}</div>
      </div>
    </>
  );
};
