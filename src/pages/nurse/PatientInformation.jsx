  ...
  return (
    <>
      ...
      <div className="wrapper">
        ...
        <div className="table">
          {loading ? <div className="empty">Loading...</div> : data.length === 0 ? <div className="empty">No patient records available.</div> : <table><thead><tr><th>Name</th><th>Email</th><th>Phone</th></tr></thead><tbody>{data.map((d) => <tr key={d.id}><td>{d.name}</td><td>{d.email}</td><td>{d.phone}</td></tr>)}</tbody></table>}
        </div>
      </div>
    </>
  );
