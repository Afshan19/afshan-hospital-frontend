  ...
  return (
    <>
      ...
      <div className="wrapper">
        ...
        <h1>Today's Queue</h1>
        <div className="table">
          {loading ? <div className="empty">Loading...</div> : data.length === 0 ? <div className="empty">No patients in queue yet.</div> : <table><thead><tr><th>Token</th><th>Patient</th><th>Room</th><th>Status</th></tr></thead><tbody>{data.map((d) => <tr key={d.id}><td>{d.token}</td><td>{d.patientName}</td><td>{d.room}</td><td>{d.status}</td></tr>)}</tbody></table>}
        </div>
      </div>
    </>
  );
