...
const DoctorDashboard = ({ onLogout, navigateTo }) => {
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        .dashboard-layout { display: flex; height: 100vh; background-color: #f4f6f9; }
        .sidebar { width: 260px; background: linear-gradient(180deg, #667eea 0%, #764ba2 100%); color: #fff; padding: 25px; display: flex; flex-direction: column; flex-shrink: 0; }
        .sidebar h2 { font-size: 22px; margin-bottom: 20px; font-weight: 800; color: #fff; }
        .nav-menu { flex: 1; display: flex; flex-direction: column; gap: 5px; overflow-y: auto; padding-right: 5px; }
        .nav-item { padding: 10px 12px; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 500; color: #fff; transition: 0.3s; }
        .nav-item:hover, .nav-item.active { background: rgba(255, 255, 255, 0.2); }
        .logout-btn { padding: 12px 16px; border-top: 1px solid rgba(255, 255, 255, 0.3); color: #fff; cursor: pointer; margin-top: 15px; }
        ...
        @media (max-width: 900px) { .dashboard-layout { flex-direction: column; } .sidebar { width: 100%; } .stats-grid { grid-template-columns: 1fr 1fr; } }
      `}</style>
      ...
      <div className="dashboard-layout">
        ...
        <div className="main-content">
          ...
          <div className="empty-state">
            ...
            <p>Select an option from the sidebar to manage your patients and appointments.</p>
          </div>
        </div>
      </div>
    </>
