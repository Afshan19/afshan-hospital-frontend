...
const DoctorLabReports = ({ navigateTo }) => {
  ...
  return (
    <>
      ...
      <div className="wrapper">
        ...
        <h1>Lab Reports</h1>
        <div className="table">
          {loading ? <div className="empty">Loading...</div> : data.length === 0 ? <div className="empty">No lab reports available.</div> : <table><thead><tr><th>Patient</th><th>Test</th><th>Result</th></tr></thead><tbody>{data.map((d) => <tr key={d.id}><td>{d.patientName}</td><td>{d.test}</td><td>{d.result}</td></tr>)}</tbody></table>}
        </div>
      </div>
    </>
  );
